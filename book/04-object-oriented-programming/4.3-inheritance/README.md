...
Inheritance lets one class reuse and extend another's behavior instead of rewriting it from scratch. A **child class** (subclass) inherits everything from its **parent class** (superclass), and can **override** any method it
wants to behave differently.

Both languages use nearly identical vocabulary: `super()` calls the parent's version of something (usually the constructor), and overriding a method just means defining a method with the same name in the child class — it takes priority automatically.
...
t = Teacher("Reagan", 34, "Physics")
print(t.introduce())          # "Hi, I'm Reagan, and I teach Physics."
print(t.name, t.age)            # "Reagan 34" -- set by the parent's __init__, via super()
